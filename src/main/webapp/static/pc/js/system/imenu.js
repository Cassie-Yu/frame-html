...
imenu.clearMenuModal = function () {
    $('#addMenuModal .page-form').removeClass('has-error');//清除模态框验证后的错误样式
	$('#addMenuModal .form-control').val('');//清空模态框中所有输入框的内容
    $('#addMenuModal .error-msg-label').html('');//清空模态框下的错误提示信息
    $('.input-radio :radio').removeAttr('checked');
    $('.input-radio input:eq(0)').prop('checked',true);//确保每次打开模态框单选都为是
}
